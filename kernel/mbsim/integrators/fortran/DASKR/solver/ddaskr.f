...
C   XERRWD/XSETF/XSETUN/IXSAV2 is a package to handle error messages. 
...
C   DATV2   computes matrix-vector product in Krylov algorithm.
...
C***REFER TO DDASKR
C***ROUTINES CALLED  DDATRP, DROOTS2, DCOPY, RT
C***REVISION HISTORY  (YYMMDD)
C   020815  DATE WRITTEN   
...
C                1/(step size H).
C
C           WK = Real work array used by routine DATV2 and PSOL.
C
...
C                matrix HES.
C
C         NRE  = The number of calls to RES (i.e. DATV2)
C
...
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   PSOL, DNRM2, DSCAL, DATV2, DORTH, DHEQR, DCOPY, DHELS, DAXPY
C
...
C-----------------------------------------------------------------------
      ...
      DO 90 LL = 1,MAXL
        LGMR = LL
C-----------------------------------------------------------------------
C Call routine DATV2 to compute VNEW = ABAR*V(LL), where ABAR is
...
C-----------------------------------------------------------------------
        CALL DATV2 (NEQ, Y, TN, YPRIME, SAVR, V(1,LL), WGHT, Z,
...
C------END OF SUBROUTINE DSPIGM-----------------------------------------
      END
      SUBROUTINE DATV2(NEQ,Y,TN,YPRIME, SAVR, V, WGHT, YPTEM, RES,
     *   IRES, PSOL, Z, VTEM, WP, IWP, CJ, EPLIN, IER, NRE, NPSL,
     *   RPAR,IPAR)
C
C***BEGIN PROLOGUE  DATV2
C***DATE WRITTEN   890101   (YYMMDD)
C***REVISION DATE  900926   (YYMMDD)
C
C
...
C Disable DHEQR DHELS as they are already provided in opkda1.f

C      SUBROUTINE DHEQR (A, LDA, N, Q, INFO, IJOB)
CC
CC***BEGIN PROLOGUE  DHEQR
...
CC***DESCRIPTION
CC
CC     This routine performs a QR decomposition of an upper
CC     Hessenberg matrix A.  There are two options available:
CC
CC          (1)  performing a fresh decomposition
CC          (2)  updating the QR factors by adding a row and A
CC               column to the matrix A.
CC
CC     DHEQR decomposes an upper Hessenberg matrix by using Givens
CC     rotations.
CC
CC     On entry
CC
CC        A       DOUBLE PRECISION(LDA, N)
CC                The matrix to be decomposed.
CC
CC        LDA     INTEGER
CC                The leading dimension of the array A.
CC
CC        N       INTEGER
CC                A is an (N+1) by N Hessenberg matrix.
CC
CC        IJOB    INTEGER
CC                = 1     Means that a fresh decomposition of the
CC                        matrix A is desired.
CC                .GE. 2  Means that the current decomposition of A
CC                        will be updated by the addition of a row
CC                        and a column.
CC     On return
CC
CC        A       The upper triangular matrix R.
CC                The factorization can be written Q*A = R, where
CC                Q is a product of Givens rotations and R is upper
CC                triangular.
CC
CC        Q       DOUBLE PRECISION(2*N)
CC                The factors C and S of each Givens rotation used
CC                in decomposing A.
CC
CC        INFO    INTEGER
CC                = 0  normal value.
CC                = K  If  A(K,K) .EQ. 0.0.  This is not an error
CC                     condition for this subroutine, but it does
CC                     indicate that DHELS will divide by zero
CC                     if called.
CC
CC     Modification of LINPACK.
CC     Peter Brown, Lawrence Livermore Natl. Lab.
CC
CC-----------------------------------------------------------------------
CC***ROUTINES CALLED (NONE)
CC
CC***END PROLOGUE  DHEQR
CC
C      INTEGER LDA, N, INFO, IJOB
C      DOUBLE PRECISION A(LDA,*), Q(*)
C      INTEGER I, IQ, J, K, KM1, KP1, NM1
C      DOUBLE PRECISION C, S, T, T1, T2
CC
C      IF (IJOB .GT. 1) GO TO 70
CC-----------------------------------------------------------------------
CC A new factorization is desired.
CC-----------------------------------------------------------------------
CC
CC     QR decomposition without pivoting.
CC
C      INFO = 0
C      DO 60 K = 1, N
C         KM1 = K - 1
C         KP1 = K + 1
CC
CC           Compute Kth column of R.
CC           First, multiply the Kth column of A by the previous
CC           K-1 Givens rotations.
CC
C            IF (KM1 .LT. 1) GO TO 20
C            DO 10 J = 1, KM1
C              I = 2*(J-1) + 1
C              T1 = A(J,K)
C              T2 = A(J+1,K)
...
C   10         CONTINUE
CC
CC           Compute Givens components C and S.
CC
C   20       CONTINUE
C            IQ = 2*KM1 + 1
C            T1 = A(K,K)
C            T2 = A(KP1,K)
C            IF (T2 .NE. 0.0D0) GO TO 30
C              C = 1.0D0
C              S = 0.0D0
C              GO TO 50
C   30       CONTINUE
C            IF (ABS(T2) .LT. ABS(T1)) GO TO 40
C              T = T1/T2
C              S = -1.0D0/SQRT(1.0D0+T*T)
C              C = -S*T
C              GO TO 50
C   40       CONTINUE
C              T = T2/T1
C              C = 1.0D0/SQRT(1.0D0+T*T)
C              S = -C*T
C   50       CONTINUE
C            Q(IQ) = C
C            Q(IQ+1) = S
C            A(K,K) = C*T1 - S*T2
C            IF (A(K,K) .EQ. 0.0D0) INFO = K
C   60 CONTINUE
C      RETURN
CC-----------------------------------------------------------------------
CC The old factorization of A will be updated.  A row and a column
CC has been added to the matrix A.
CC N by N-1 is now the old size of the matrix.
CC-----------------------------------------------------------------------
C  70  CONTINUE
C      NM1 = N - 1
CC-----------------------------------------------------------------------
CC Multiply the new column by the N previous Givens rotations.
CC-----------------------------------------------------------------------
C      DO 100 K = 1,NM1
C        I = 2*(K-1) + 1
C        T1 = A(K,N)
...
C 100    CONTINUE
CC-----------------------------------------------------------------------
CC Complete update of decomposition by forming last Givens rotation,
CC and multiplying it times the column vector (A(N,N),A(NP1,N)).
CC-----------------------------------------------------------------------
C      INFO = 0
C      T1 = A(N,N)
C      T2 = A(N+1,N)
C      IF (T2 .NE. 0.0D0) GO TO 110
C        C = 1.0D0
C        S = 0.0D0
C        GO TO 130
C 110  CONTINUE
C      IF (ABS(T2) .LT. ABS(T1)) GO TO 120
C        T = T1/T2
C        S = -1.0D0/SQRT(1.0D0+T*T)
C        C = -S*T
C        GO TO 130
...
C      RETURN
CC
CC------END OF SUBROUTINE DHEQR------------------------------------------
C      END
C      SUBROUTINE DHELS (A, LDA, N, Q, B)
CC
CC***BEGIN PROLOGUE  DHELS
CC***DATE WRITTEN   890101   (YYMMDD)
CC***REVISION DATE  900926   (YYMMDD)
CC
CC
...
CC***DESCRIPTION
CC
CC This is similar to the LINPACK routine DGESL except that
CC A is an upper Hessenberg matrix.
CC
CC     DHELS solves the least squares problem
CC
CC           MIN (B-A*X,B-A*X)
CC
CC     using the factors computed by DHEQR.
CC
CC     On entry
CC
CC        A       DOUBLE PRECISION (LDA, N)
CC                The output from DHEQR which contains the upper
CC                triangular factor R in the QR decomposition of A.
CC
CC        LDA     INTEGER
CC                The leading dimension of the array  A .
CC
CC        N       INTEGER
CC                A is originally an (N+1) by N matrix.
CC
CC        Q       DOUBLE PRECISION(2*N)
CC                The coefficients of the N givens rotations
CC                used in the QR factorization of A.
CC
CC        B       DOUBLE PRECISION(N+1)
...
CC   DAXPY 
CC
CC***END PROLOGUE  DHELS
CC
C      INTEGER LDA, N
C      DOUBLE PRECISION A(LDA,*), B(*), Q(*)
C      INTEGER IQ, K, KB, KP1
C      DOUBLE PRECISION C, S, T, T1, T2
CC
CC        Minimize (B-A*X,B-A*X).
CC        First form Q*B.
CC
C         DO 20 K = 1, N
C            KP1 = K + 1
